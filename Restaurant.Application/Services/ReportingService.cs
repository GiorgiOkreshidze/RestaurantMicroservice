    ...
    ILocationRepository locationRepository) : IReportingService
{
    ...
    public async Task<ReportResponse> GetReportsAsync(ReportRequest request)
    {
        ...
        logger.LogInformation("Retrieved waiterSummaries {WaiterSummaries}", JsonSerializer.Serialize(waiterSummaries));
        logger.LogInformation("Retrieved locationSummaries {LocationSummaries}", JsonSerializer.Serialize(locationSummaries));
