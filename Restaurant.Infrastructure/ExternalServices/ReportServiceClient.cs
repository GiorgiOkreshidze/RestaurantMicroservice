...
public class ReportServiceClient(HttpClient httpClient, ILogger<ReportServiceClient> logger) : IReportServiceClient
{
    ...
    public async Task<(List<WaiterSummaryResponse> WaiterSummaries, List<LocationSummaryResponse> LocationSummaries)> GetReportsAsync(
        string baseUrl,
        string startDate,
        string endDate,
        string? locationId,
        string? reportType = null)
    {
        ...
        var waiterSummaries = new List<WaiterSummaryResponse>();
        ...
        try
        {
                 ...
                 root.TryGetProperty("performance", out var performanceElement)))
            {
                ...
                {
                    waiterSummaries = JsonSerializer.Deserialize<List<WaiterSummaryResponse>>(
                        performanceElement.GetRawText(), options) ?? new List<WaiterSummaryResponse>();
                }
            }
            ...
            {
                waiterSummaries = JsonSerializer.Deserialize<List<WaiterSummaryResponse>>(content, options) ?? 
                                  new List<WaiterSummaryResponse>();
            }
        }
